...
interface DetalleBlExcel {
  ...
  ref?: string;
  bl?: string;
  code_new?: string;    // Opcional
  codigovhcr?: string;  // Opcional
}
...
@Component({
  selector: 'app-bl-invoiceexcel',
  templateUrl: './bl-invoiceexcel.component.html',
  styleUrls: ['./bl-invoiceexcel.component.css']
})
export class BlInvoiceexcelComponent implements OnInit, OnDestroy {
  ...
  private readonly COLUMN_MAPPINGS = {
    ...
    CODE_NEW:    ['CODE_NEW', 'CODE NEW', 'NUEVO_CODIGO', 'NUEVO CODIGO'],
    CODIGOVHCR:  ['CODIGOVHCR', 'CODIGO VHCR', 'CODIGO_VHCR', 'VHCR']
  };
  ...
  private getColumnMappings(): any {
    return {
      ...
      unitCostCol:   this.findColumnName(this.fileData, this.COLUMN_MAPPINGS.UNIT_FOB),
      ...
      codeNewCol:    this.findColumnName(this.fileData, this.COLUMN_MAPPINGS.CODE_NEW),
      codigoVhcrCol: this.findColumnName(this.fileData, this.COLUMN_MAPPINGS.CODIGOVHCR)
    };
  }
  ...
  private preparePreviewData(): void {
    ...
    this.previewData = this.fileData.map(row => {
      ...
      if (columnMappings.codeNewCol && row[columnMappings.codeNewCol]) {
        detalle.code_new = String(row[columnMappings.codeNewCol]);
      }
      if (columnMappings.codigoVhcrCol && row[columnMappings.codigoVhcrCol]) {
        detalle.codigovhcr = String(row[columnMappings.codigoVhcrCol]);
      }
  ...
  private parseNumber(value: any): number {
    ...
    // Si xlsx ya parseó el valor como número, usarlo directamente sin convertir a string
    // (evita que replace destruya el separador decimal: 18.55 → "18.55" → replace('.','') → 1855)
    if (typeof value === 'number') return isNaN(value) ? 0 : value;

    // Es string: limpiar símbolo de moneda y espacios
    const s = String(value).replace(/[$\s]/g, '').trim();
    if (s === '') return 0;

    // Formato europeo con puntos de miles y coma decimal: "1.234,56"
    if (/\d+\.\d{3},\d+/.test(s) || (/,/.test(s) && /\./.test(s) && s.indexOf(',') > s.indexOf('.'))) {
      return parseFloat(s.replace(/\./g, '').replace(',', '.')) || 0;
    }
    // Solo coma como decimal (sin puntos de miles): "18,55"
    if (/^\d+,\d+$/.test(s)) {
    ...
    // Número directo con punto decimal o entero: "18.55", "1855"
  ...
  downloadTemplate(): void {
    ...
    const instructionsData = [
      ...
      ['   • ORDEN: Fecha estimada de llegada'],
      ['   • REF: Referencia'],
