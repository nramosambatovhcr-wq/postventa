...
@Component({
  selector: 'app-agenciasbod',
  templateUrl: './agenciasbod.component.html',
  styleUrls: ['./agenciasbod.component.css']
})
export class AgenciasbodComponent implements OnInit {
  ...
  goToRevision(oficinaId: string): void {
    ...
    if (this.usrol === 'bodegaimpor' || this.usrol === 'admin'|| this.usrol === 'bodegaimpor3') {
      this.router.navigate(['/revisionpartes', oficinaId]);
    }
    if (this.usrol === 'bodegaimpor1') {
      this.router.navigate(['/analisisbodag', oficinaId]);
    }
  }
}