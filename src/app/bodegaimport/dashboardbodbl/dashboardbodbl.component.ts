...
@Component({
  selector: 'app-dashboardbodbl',
  templateUrl: './dashboardbodbl.component.html',
  styleUrls: ['./dashboardbodbl.component.css']
})
export class DashboardbodblComponent implements OnInit, OnDestroy {
  stats = {
...
verBl(blId: number){
 this.router.navigate(['/dashboardconteo', blId]);
}
updatebl(blId: number){
 this.router.navigate(['/blrevupdate', blId]);
}
...
/*
  ...
  // En dashboardbodbl.component.ts

searchImports(): void {
    const lowerCaseSearchTerm = this.searchTerm.toLowerCase().trim();
    if (!lowerCaseSearchTerm) {
        this.filteredData = [...this.bls];
    } else {
        this.filteredData = this.bls.filter(item =>
            (item.nombre && item.nombre.toLowerCase().includes(lowerCaseSearchTerm)) ||
...
*/
  ...
  navigate2(orden: any): void {
    console.log(orden);
    this.router.navigate(['/dashboardconteo', orden]);
  }
 ...
 downloadExcel(): void {
  const dataToExport = this.filteredData.map(item => ({
      ...
      estado: item.estado,
      fecha_creacion: item.fecha_creacion ? new Date(item.fecha_creacion).toLocaleDateString() : '',
      fecha_arribo: item.farrivop ? new Date(item.farrivop).toLocaleDateString() : 'N/A',
      fecha_embarque: item.fembarque ? new Date(item.fembarque).toLocaleDateString() : 'N/A',
      fecha_transito: item.ftransito ? new Date(item.ftransito).toLocaleDateString() : 'N/A',
      fecha_aduana: item.faduana ? new Date(item.faduana).toLocaleDateString() : 'N/A',
      fecha_liquidacion: item.fliquidacion ? new Date(item.fliquidacion).toLocaleDateString() : 'N/A',
...
// Update downloadCSV method to include arrival date
downloadCSV(): void {
  let csvContent = 'Cod,BL,Proveedor,Invoice Principal,Estado,Fecha Creacion,Fecha Arribo,Fecha Embarque,Fecha Transito,Fecha Aduana,Fecha Liquidacion,Descripcion BL,Total Invoices,InvoiceBL Asociados\n';
  ...
  this.filteredData.forEach(item => {
    ...
    const row = [
      ...
      item.farrivop ? new Date(item.farrivop).toLocaleDateString() : 'N/A',
      item.fembarque ? new Date(item.fembarque).toLocaleDateString() : 'N/A',
      item.ftransito ? new Date(item.ftransito).toLocaleDateString() : 'N/A',
      item.faduana ? new Date(item.faduana).toLocaleDateString() : 'N/A',
      item.fliquidacion ? new Date(item.fliquidacion).toLocaleDateString() : 'N/A',
      item.descripcion || '',
...
// Helper method to get status-based color for dates
