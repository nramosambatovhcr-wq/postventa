...
@Component({
  selector: 'app-dashboardbodimp',
  templateUrl: './dashboardbodimp.component.html',
  styleUrls: ['./dashboardbodimp.component.css']
})
export class DashboardbodimpComponent implements OnInit, OnDestroy  {
  ...
  showNuevoBuscador:boolean = false;
openNuevoBuscador(): void {
  ...
  actualizarEstado(item: Pedido): void {
    ...
    const pedidoActualizado = {
      ...
      Estado: item.estado || "",
      IdUsuarioCreacion: item.idUsuarioCreacion || 0,
      IdUsuarioModificacion: item.idUsuarioModificacion || 0,
      Modelo: item.modelo || "",
  ...
  // --- MÉTODOS DE FILTRADO Y PAGINACIÓN OPTIMIZADOS ---
  ...
  searchImports(): void {
    // Este método ahora simplemente llama a la función principal de filtrado
    this.applyFiltersAndPagination();
  }
  ...
  // --- FIN DE MÉTODOS DE FILTRADO Y PAGINACIÓN OPTIMIZADOS ---
