...
@Component({
  selector: 'app-dashboardcot',
  templateUrl: './dashboardcot.component.html',
  styleUrls: ['./dashboardcot.component.css']
})
export class DashboardcotComponent  implements OnInit, OnDestroy {
  searchTerm: string = '';
  ...
  loadimportacionesus() {
    this.loading = true;
    this.tutorialService.getCotizacionByUser(this.id).subscribe({
      next: (data: any) => {
        console.log(data.length);
        this.allData = data;
        this.extractUniqueValues();
  ...
  // ─── Cancelar cotización (cambia estado a Rechazada) ─────────────────────────
  cancelarCotizacion(item: any): void {
    if (item._procesando) return;

    const confirmado = confirm(
      `Anular la cotización "${item.codigoCot}"?\n\nCambiará su estado a "Anulada". Esta acción se puede revertir cambiando el estado manualmente.`
    );
