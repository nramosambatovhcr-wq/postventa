...
@Component({
  selector: 'app-blpro',
  templateUrl: './blpro.component.html',
  styleUrls: ['./blpro.component.css']
})
export class BlproComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.authService.usuarioActual$.subscribe(usuario => {
      ...
      if(this.usuario != null){
        ...
        this.idprov = Number(this.usuario.agencia);
        console.log(this.id);
        this.loadAll();
      }
    });
...
verBl(blId: number){
 this.router.navigate(['/archivosbl', blId]);
}
...
verdetail(blId: number){
   this.router.navigate(['/blprodetail', blId]);
  //blvhcrdetail
}
  ...
  loadAllBls(): void {
    this.loading = true;
    this.blService.getAllBlProveedorTra(this.idprov).subscribe({
  ...
  loadAll(): void {
    this.loading = true;
    this.blService.getAllBlProveedor(this.idprov).subscribe({
      next: (data: BlRawResponse[]) => {
        console.log('Datos recibidos del backend:', data);
  ...
  crear(): void {
    this.router.navigate(['/blprocreate']);
  }

  excel(): void {
    this.router.navigate(['/blprocreateexcel']);
  }
  ...
  applyFilters(): void {
    ...
    // AQUI SE CORRIGE LA ASIGNACION PARA QUE SOLO MUESTRE LA PRIMERA PAGINA DESPUES DEL FILTRO
