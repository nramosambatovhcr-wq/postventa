...
@Component({
  selector: 'app-cotasignada',
  templateUrl: './cotasignada.component.html',
  styleUrls: ['./cotasignada.component.css']
})
export class CotasignadaComponent implements OnInit, OnDestroy {
  stats = {
  ...
  this.tutorialService.getCotizacionProByUser(this.id).subscribe({
    next: (data: any) => {
      ...
      this.allData = data.filter((item:any) =>
        item.estadoCotizacion?.toLowerCase() === 'asignada'
      );
