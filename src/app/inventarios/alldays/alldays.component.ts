...
export class AlldaysComponent implements OnInit {
  ...
  generarAnalisisVarianzas(): void {
    this.analisisVarianzas = this.articulosDiarios.map(articulo => {
      ...
      this.gruposAnalisis.forEach(grupo => {
        ...
        const isReconteo = dias.includes('10') || dias.includes('11');
        
        const tieneConteo = this.tieneConteoEnDias(articulo, dias);

        if (tieneConteo) {
          const sumaContada = isReconteo
            ? this.getSumaReconteosPorDias(articulo, dias)
            : this.getSumaConteosPorDias(articulo, dias);

          const detalle = this.getDetalleVarianzasPorDias(articulo, dias);
          const diferenciaTotal = sumaContada - stockDisponible;
          const observaciones = this.getObservacionesPorDias(articulo, dias);
  ...
  exportarAnalisisVarianzas1(): void {
...
getArticulosFDIConCosto(): AnalisisVarianzaArticulo[] {
 ...
 /**
 * MÉTODO EXPORTAR PDF OPTIMIZADO
 * Reemplaza el método exportarPDF() existente en alldays.component.ts
 * Mejoras: Mayor velocidad, mejor calidad, captura completa hasta firmas
 */
...
exportarPDF1(): void {
  ...
  loadingDiv.innerHTML = `
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
                background: rgba(0,0,0,0.7); z-index: 99998; display: flex; 
      ...
      <div style="background: white; padding: 40px; border-radius: 16px; 
        ...
        <h4 style="color: #333; margin-bottom: 12px; font-weight: 600;">
          Generando Informe PDF
        </h4>
        <p style="color: #666; font-size: 14px; margin-bottom: 8px;">
          Procesando contenido completo...
        </p>
        <p style="color: #999; font-size: 12px;">
          Este proceso puede tardar 10-20 segundos
        </p>
        <div style="margin-top: 20px; background: #f0f0f0; border-radius: 8px; 
                    height: 8px; overflow: hidden;">
          <div id="pdf-progress-bar" style="height: 100%; background: linear-gradient(90deg, #0066cc, #0099ff); 
  ...
  setTimeout(() => {
    ...
    // PASO 1: Identificar el contenedor principal
    const element = document.querySelector('.container-fluid') as HTMLElement;
    
    if (!element) {
      console.error('No se encontró el contenedor principal');
      document.body.removeChild(loadingDiv);
      this.mostrarMensajeError('Error: No se encontró el contenido para exportar');
      return;
    }

    updateProgress(20);

    // PASO 2: Preparar el contenedor para captura
    ...
    // PASO 3: Ocultar botones de exportación temporalmente
    const exportButtons = element.querySelectorAll('.btn-export, .export-buttons-group, .no-print');
    const hiddenElements: HTMLElement[] = [];
    
    exportButtons.forEach(btn => {
      const htmlBtn = btn as HTMLElement;
      if (htmlBtn.style.display !== 'none') {
        htmlBtn.style.display = 'none';
        hiddenElements.push(htmlBtn);
      }
    });

    updateProgress(30);

    // PASO 4: Configuración optimizada de html2canvas
    const canvasOptions = {
      scale: 2, // Calidad HD (reducido de 3 para mayor velocidad)
      useCORS: true,
      allowTaint: false,
      logging: false,
      imageTimeout: 0,
      ...
      onclone: (clonedDoc: Document) => {
        const clonedElement = clonedDoc.querySelector('.container-fluid') as HTMLElement;
        if (clonedElement) {
          ...
          const noprint = clonedElement.querySelectorAll('.no-print, .btn-export');
    ...
    // PASO 5: Generar canvas con html2canvas
    html2canvas(element, canvasOptions as any)
      .then(canvas => {
        updateProgress(60);

        // PASO 6: Calcular dimensiones para PDF
        const imgWidth = 210; // A4 width en mm
        const pageHeight = 297; // A4 height en mm
        const imgHeight = (canvas.height * imgWidth) / canvas.width;
        
        // PASO 7: Crear PDF con orientación apropiada
        const pdf = new jsPDF({
        ...
        // PASO 8: Convertir canvas a imagen
        const imgData = canvas.toDataURL('image/jpeg', 0.85); // JPEG con 85% calidad para menor tamaño

        // PASO 9: Agregar imagen al PDF con paginación automática
        let heightLeft = imgHeight;
        let position = 0;
        const pageHeightWithMargin = pageHeight - 20; // Márgenes de 10mm arriba y abajo
        ...
        // PASO 10: Generar metadatos y nombre del archivo
        const fecha = new Date();
        const fechaStr = fecha.toLocaleDateString('es-EC').replace(/\//g, '-');
        const horaStr = fecha.toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit' }).replace(/:/g, '-');
        
        const nombreArchivo = `Informe-Inventario-${this.getNombreBodega()}-${fechaStr}-${horaStr}.pdf`;

        // Agregar metadatos al PDF
        pdf.setProperties({
          title: `Informe de Inventario Físico - ${this.getNombreBodega()}`,
          subject: 'Análisis de Varianzas de Inventario',
          author: this.getResponsablesConteo(),
          keywords: 'inventario, física, conteo, varianzas, faltantes, sobrantes',
          creator: 'Sistema de Gestión de Inventarios'
        });
        ...
        // PASO 12: Limpiar y restaurar
        setTimeout(() => {
          // Restaurar estilos originales
          element.style.overflow = originalOverflow;
          element.style.height = originalHeight;

          // Restaurar botones ocultos
  ...
  }, 200); // Delay inicial para renderizar loading
}

/**
 * MÉTODO EXPORTAR PDF OPTIMIZADO - SOLO HASTA FIRMAS
 * Reemplaza el método exportarPDF() existente en alldays.component.ts
 */
...
exportarPDF2(): void {
  ...
  loadingDiv.innerHTML = `
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
      ...
      <div style="background: white; padding: 40px; border-radius: 16px; 
        ...
        <p style="color: #666; font-size: 14px; margin-bottom: 8px;">
          Procesando contenido hasta firmas...
        </p>
        <p style="color: #999; font-size: 12px;">
          Este proceso puede tardar 10-20 segundos
        </p>
  ...
  setTimeout(() => {
    ...
    // PASO 1: Obtener el contenedor principal
    const mainContainer = document.querySelector('.container-fluid') as HTMLElement;
    ...
    // PASO 2: Crear contenedor temporal con SOLO el contenido hasta firmas
    const tempContainer = document.createElement('div');
    tempContainer.className = 'container-fluid p-0';
    tempContainer.style.backgroundColor = '#ffffff';
    tempContainer.style.padding = '20px';
    tempContainer.style.width = '210mm'; // Ancho A4
    
    // Seleccionar SOLO las secciones hasta documento confidencial
    const allSections = mainContainer.querySelectorAll(
      '.main-header, .content-section, .firmas-section, .documento-confidencial'
    );
    ...
    allSections.forEach(section => {
      if (!foundEnd) {
        ...
        const exportButtons = clonedSection.querySelectorAll('.btn-export, .export-buttons-group');
        exportButtons.forEach(btn => {
        ...
        if (section.classList.contains('documento-confidencial')) {
          foundEnd = true;
        }
      }
    });
    ...
    // PASO 3: Agregar el contenedor temporal al body (necesario para html2canvas)
    ...
    const canvasOptions = {
      ...
      onclone: (clonedDoc: Document) => {
        ...
        if (clonedElement) {
          ...
          // Ocultar todos los botones en el clon
          const buttons = clonedElement.querySelectorAll('.btn-export, .export-buttons-group, .no-print');
    ...
    // PASO 5: Generar canvas
    html2canvas(tempContainer, canvasOptions as any)
      .then(canvas => {
        updateProgress(70);

        // PASO 6: Remover contenedor temporal
        document.body.removeChild(tempContainer);

        // PASO 7: Calcular dimensiones para PDF
        const imgWidth = 210; // A4 width en mm
        const pageHeight = 297; // A4 height en mm
        const imgHeight = (canvas.height * imgWidth) / canvas.width;
        ...
        // PASO 9: Convertir canvas a imagen
        const imgData = canvas.toDataURL('image/jpeg', 0.90); // 90% calidad

        // PASO 10: Agregar imagen al PDF con paginación
        let heightLeft = imgHeight;
        let position = 0;
        const marginTop = 10;
        const marginBottom = 10;
        const pageHeightWithMargin = pageHeight - marginTop - marginBottom;

        // Primera página
        pdf.addImage(imgData, 'JPEG', 0, position + marginTop, imgWidth, imgHeight);
        heightLeft -= pageHeightWithMargin;
        ...
        // PASO 11: Generar metadatos y nombre del archivo
        const fecha = new Date();
        const fechaStr = fecha.toLocaleDateString('es-EC').replace(/\//g, '-');
        ...
        const nombreArchivo = `Informe-Ejecutivo-${this.getNombreBodega()}-${fechaStr}-${horaStr}.pdf`;

        // Agregar metadatos
        pdf.setProperties({
          title: `Informe Ejecutivo de Inventario - ${this.getNombreBodega()}`,
          subject: 'Análisis de Inventario Físico hasta Firmas y Aprobaciones',
          author: this.getResponsablesConteo(),
          keywords: 'inventario, ejecutivo, análisis, firmas, aprobaciones',
          creator: 'Sistema de Gestión de Inventarios VEHICENTRO'
        });
        ...
        // PASO 13: Limpiar y mostrar mensaje de éxito
        setTimeout(() => {
          document.body.removeChild(loadingDiv);
          
          this.mostrarMensajeExito(
            `✅ PDF Ejecutivo generado exitosamente<br>
            <small>Archivo: ${nombreArchivo}</small><br>
            <small class="text-muted">Contenido: Informe hasta Firmas y Aprobaciones</small>`
          );
        }, 500);
      ...
      .catch(error => {
        ...
        const tempContainerCheck = document.querySelector('.container-fluid[style*="-9999px"]');
        if (tempContainerCheck && document.body.contains(tempContainerCheck)) {
          document.body.removeChild(tempContainerCheck);
        }
...
/**
 * MÉTODO EXPORTAR PDF OPTIMIZADO - SIN BLOQUEO DEL NAVEGADOR
 * Versión mejorada que procesa de forma asíncrona y no congela la página
 * Tiempo estimado: 5-8 segundos
 */

exportarPDF(): void {
  // Crear indicador de carga
  const loadingDiv = this.crearLoadingMejorado();
  document.body.appendChild(loadingDiv);

  // Usar requestAnimationFrame para no bloquear el UI
  requestAnimationFrame(() => {
    this.procesarPDFAsincrono(loadingDiv);
  });
}

private async procesarPDFAsincrono(loadingDiv: HTMLElement): Promise<void> {
  try {
    this.actualizarProgreso(5, 'Preparando contenido...');

    // PASO 1: Obtener contenedor principal
    const mainContainer = document.querySelector('.container-fluid') as HTMLElement;
    if (!mainContainer) {
      throw new Error('No se encontró el contenedor principal');
    }

    await this.esperarFrame(); // No bloquear UI
    this.actualizarProgreso(10, 'Clonando secciones...');

    // PASO 2: Crear contenedor temporal OPTIMIZADO
    const tempContainer = this.crearContenedorOptimizado(mainContainer);
    
    await this.esperarFrame();
    this.actualizarProgreso(20, 'Aplicando estilos...');

    // PASO 3: Configuración ULTRA-OPTIMIZADA
    const canvasOptions = {
      scale: 1.3,  // Balance óptimo calidad/velocidad
      useCORS: true,
      allowTaint: false,
      logging: false,
      imageTimeout: 0,
      backgroundColor: '#ffffff',
      removeContainer: false, // Evitar operaciones DOM extras
      foreignObjectRendering: false, // Más rápido
      width: tempContainer.scrollWidth,
      height: tempContainer.scrollHeight,
      windowWidth: tempContainer.scrollWidth,
      windowHeight: tempContainer.scrollHeight,
    ...
    this.actualizarProgreso(30, 'Renderizando contenido...');
    await this.esperarFrame();

    // PASO 4: Generar canvas de forma asíncrona
    const canvas = await html2canvas(tempContainer, canvasOptions as any);
    
    this.actualizarProgreso(65, 'Creando PDF...');
    await this.esperarFrame();

    // PASO 5: Limpiar DOM inmediatamente
    if (document.body.contains(tempContainer)) {
      document.body.removeChild(tempContainer);
    }

    // PASO 6: Crear PDF de forma optimizada
    const pdf = await this.crearPDFOptimizado(canvas);
    
    this.actualizarProgreso(90, 'Guardando archivo...');
    await this.esperarFrame();

    // PASO 7: Generar nombre y guardar
    const nombreArchivo = this.generarNombreArchivo('Informe-Ejecutivo');
    pdf.save(nombreArchivo);

    this.actualizarProgreso(100, 'Completado');
    
    // PASO 8: Mostrar éxito y limpiar
    await this.esperarFrame();
    this.finalizarExito(loadingDiv, nombreArchivo);

  } catch (error: any) {
    console.error('Error al generar PDF:', error);
    this.finalizarError(loadingDiv, error.message);
  }
}

/**
 * Crear contenedor optimizado con solo el contenido necesario
 */
private crearContenedorOptimizado(mainContainer: HTMLElement): HTMLElement {
  const tempContainer = document.createElement('div');
  ...
  // Seleccionar y clonar SOLO las secciones necesarias
  const sectionsToInclude = [
    '.main-header',
    '.content-section:not(:has(.table-varianzas))', // Excluir tabla grande de varianzas
    '.firmas-section',
    '.documento-confidencial'
  ];

  let foundEnd = false;
  const allSections = mainContainer.querySelectorAll('.main-header, .content-section, .firmas-section, .documento-confidencial');

  allSections.forEach((section) => {
    if (!foundEnd) {
      // Verificar si es una sección de listado completo (excluir)
      const sectionElement = section as HTMLElement;
      const hasListadoCompleto = sectionElement.textContent?.includes('LISTADO COMPLETO') || false;
      const hasTablaVarianzas = sectionElement.querySelector('.table-varianzas') !== null;
      const hasVistaDiaDia = sectionElement.textContent?.includes('Vista Día a Día') || false;

      // Solo clonar si NO es una sección de detalle masivo
      if (!hasListadoCompleto && !hasTablaVarianzas && !hasVistaDiaDia) {
        const clonedSection = section.cloneNode(true) as HTMLElement;
        
        // Limpiar elementos no necesarios
        this.limpiarSeccionClonada(clonedSection);
...
/**
 * Limpiar sección clonada de elementos innecesarios
 */
private limpiarSeccionClonada(section: HTMLElement): void {
  // Remover botones y elementos no imprimibles
  const elementosRemover = section.querySelectorAll(
    '.btn-export, .export-buttons-group, .no-print, button, .spinner-border'
  );
  elementosRemover.forEach(el => el.remove());

  // Simplificar tablas muy grandes (mantener solo primeras 50 filas)
  const tablas = section.querySelectorAll('table tbody');
  tablas.forEach(tbody => {
    const filas = Array.from(tbody.querySelectorAll('tr'));
    if (filas.length > 50) {
      filas.slice(50).forEach(fila => fila.remove());
    }
  });
}

/**
 * Optimizar documento clonado por html2canvas
 */
private optimizarDocumentoClonado(clonedDoc: Document): void {
  ...
  const buttons = clonedContainer.querySelectorAll('button, .btn-export, .no-print');
  buttons.forEach(btn => (btn as HTMLElement).style.display = 'none');

  // Optimizar imágenes
  const imgs = clonedContainer.querySelectorAll('img');
  imgs.forEach(img => {
...
/**
 * Crear PDF de forma optimizada
 */
private async crearPDFOptimizado(canvas: HTMLCanvasElement): Promise<jsPDF> {
  ...
  // Primera página
  pdf.addImage(imgData, 'JPEG', 0, margin, imgWidth, imgHeight, undefined, 'FAST');
  ...
  while (heightLeft > 0) {
    position = heightLeft - imgHeight;
    pdf.addPage();
    pdf.addImage(imgData, 'JPEG', 0, position + margin, imgWidth, imgHeight, undefined, 'FAST');
    heightLeft -= (pageHeight - margin * 2);
  }

  // Metadatos
  pdf.setProperties({
    title: `Informe Ejecutivo - ${this.getNombreBodega()}`,
    subject: 'Análisis de Inventario Físico',
    author: this.getResponsablesConteo(),
    creator: 'Sistema VEHICENTRO',
    keywords: 'inventario, análisis, firmas'
  });
...
/**
 * Esperar un frame de animación (no bloquear UI)
 */
...
/**
 * Crear loading mejorado
 */
private crearLoadingMejorado(): HTMLElement {
  const loadingDiv = document.createElement('div');
  loadingDiv.id = 'pdf-loading-overlay';
  loadingDiv.innerHTML = `
    <div style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; 
      ...
      <div style="background: white; padding: 40px 50px; border-radius: 20px; 
                  box-shadow: 0 10px 40px rgba(0,0,0,0.3); text-align: center; 
                  max-width: 450px; animation: slideIn 0.3s ease-out;">
        <div style="margin-bottom: 25px;">
          <div class="spinner-border text-primary" role="status" 
               style="width: 5rem; height: 5rem; border-width: 0.5rem;"></div>
        </div>
        <h3 id="pdf-loading-title" style="color: #333; margin-bottom: 10px; font-weight: 700; font-size: 22px;">
          Generando PDF Ejecutivo
        </h3>
        <p id="pdf-loading-message" style="color: #666; font-size: 15px; margin-bottom: 20px;">
          Preparando contenido...
        </p>
        <div style="background: #e9ecef; border-radius: 10px; height: 10px; overflow: hidden; margin-top: 20px;">
          <div id="pdf-progress-bar" style="height: 100%; 
               background: linear-gradient(90deg, #0066cc, #00b4d8); 
               width: 0%; transition: width 0.4s ease; border-radius: 10px;"></div>
        </div>
        <p id="pdf-progress-text" style="color: #999; font-size: 13px; margin-top: 12px;">
          0%
        </p>
      </div>
    </div>
 ...
 */
private actualizarProgreso(percent: number, mensaje: string): void {
  const progressBar = document.getElementById('pdf-progress-bar');
 ...
 */
private generarNombreArchivo(prefijo: string): string {
  const fecha = new Date();
  const fechaStr = fecha.toLocaleDateString('es-EC').replace(/\//g, '-');
  const horaStr = fecha.toLocaleTimeString('es-EC', { 
    hour: '2-digit', 
  ...
  return `${prefijo}-${this.getNombreBodega()}-${fechaStr}-${horaStr}.pdf`;
}

/**
 * Finalizar con éxito
 */
private finalizarExito(loadingDiv: HTMLElement, nombreArchivo: string): void {
  setTimeout(() => {
    if (document.body.contains(loadingDiv)) {
      document.body.removeChild(loadingDiv);
    }
    this.mostrarMensajeExito(
      `✅ PDF generado exitosamente<br>
      <small style="font-size: 13px; opacity: 0.9;">Archivo: ${nombreArchivo}</small><br>
      <small style="font-size: 12px; opacity: 0.7;">Contenido hasta Firmas y Aprobaciones</small>`
    );
  }, 600);
}
 ...
 * Finalizar con error
 */
private finalizarError(loadingDiv: HTMLElement, mensaje: string): void {
  if (document.body.contains(loadingDiv)) {
    document.body.removeChild(loadingDiv);
  }
  this.mostrarMensajeError(
    ...
    <small style="font-size: 11px; opacity: 0.7;">Intenta recargar la página y generar nuevamente</small>`
  );
}

/**
 * MÉTODO AUXILIAR: Exportar PDF con opciones avanzadas
 * Permite exportar secciones específicas del informe
 */
exportarPDFSeccion(seccionId: string, nombreSeccion: string): void {
  const loadingDiv = document.createElement('div');
  loadingDiv.innerHTML = `
    <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); 
      ...
      <p style="font-weight: 600;">Generando sección: ${nombreSeccion}</p>
    </div>
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
  ...
  setTimeout(() => {
    const element = document.getElementById(seccionId) as HTMLElement;
    
    if (!element) {
      document.body.removeChild(loadingDiv);
      this.mostrarMensajeError(`No se encontró la sección: ${nombreSeccion}`);
      return;
    }
    ...
    html2canvas(element, {
      ...
      pdf.save(`${nombreSeccion}-${fecha}.pdf`);

      document.body.removeChild(loadingDiv);
      this.mostrarMensajeExito(`PDF de ${nombreSeccion} generado`);
    }).catch(error => {
      console.error('Error:', error);
      document.body.removeChild(loadingDiv);
      this.mostrarMensajeError('Error al generar PDF de sección');
    });
  }, 200);
}

/**
 * MÉTODO MEJORADO: Mostrar mensaje de éxito con HTML
 */
...
/**
 * Exportar a HTML interactivo completo (hasta sección de firmas)
 */
exportarHTML(): void {
  const element = document.querySelector('.container-fluid') as HTMLElement;
  ...
  // Crear un contenedor temporal para clonar solo el contenido deseado
  const tempContainer = document.createElement('div');
  tempContainer.className = 'container-fluid p-0';

  // Seleccionar SOLO las secciones hasta "Firmas y Aprobaciones"
  const allSections = element.querySelectorAll('.content-section, .main-header, .firmas-section, .documento-confidencial');
  let foundEnd = false;
  ...
  allSections.forEach(section => {
    if (!foundEnd) {
      ...
      // Detectar si llegamos al final deseado (después de documento confidencial)
      if (section.classList.contains('documento-confidencial')) {
        foundEnd = true;
      }
    }
  ...
  const clonedElement = tempContainer;
...
/**
 * MÉTODOS PARA ARTÍCULOS CONCILIADOS
 */

/**
 * Obtener artículos conciliados (diferencia = 0)
 */
getArticulosConciliados(): AnalisisVarianzaArticulo[] {
  return this.analisisVarianzas.filter(item => 
    item.diferenciaFinal === 0 && item.conteoFinalAceptado > 0
  );
}

/**
 * Calcular total de unidades conciliadas
 */
calcularTotalUnidadesConciliadas(): number {
  return this.getArticulosConciliados().reduce((total, item) => {
    return total + item.stock_disponible;
  }, 0);
}

/**
 * Obtener valor total de artículos conciliados
 */
getValorTotalConciliados(): number {
  return this.getArticulosConciliados().reduce((total, item) => {
    return total + (item.stock_disponible * item.costo_promedio);
  }, 0);
}

/**
 * Obtener porcentaje de artículos conciliados
 */
getPorcentajeConciliados(): number {
  const total = this.analisisVarianzas.length;
  if (total === 0) return 0;
  return (this.getArticulosConciliados().length / total) * 100;
}

/**
 * Análisis de artículos conciliados por grupo
 */
getAnalisisConciliadosPorGrupo(): any[] {
  ...
  this.articulosDiarios.forEach(art => {
    ...
    if (analisisItem && analisisItem.diferenciaFinal === 0 && analisisItem.conteoFinalAceptado > 0) {
      if (!grupos.has(grupo)) {
        grupos.set(grupo, {
          grupo: grupo,
          grupoNombre: art.grupo || 'Sin Grupo',
          articulosConciliados: 0,
          unidadesConciliadas: 0,
          valorConciliado: 0
        });
      }
      
      const grupoData = grupos.get(grupo)!;
      grupoData.articulosConciliados++;
      grupoData.unidadesConciliadas += analisisItem.stock_disponible;
      grupoData.valorConciliado += (analisisItem.stock_disponible * analisisItem.costo_promedio);
    }
  });
  
  return Array.from(grupos.values())
    .sort((a, b) => b.valorConciliado - a.valorConciliado);
}

/**
 * Exportar a Excel el listado completo de artículos CONCILIADOS
 */
exportarExcelConciliados(): void {
  const articulosConciliados = this.getArticulosConciliados();
  
  if (articulosConciliados.length === 0) {
    this.mostrarMensajeError('No hay artículos conciliados para exportar');
    return;
  }
  ...
  // ENCABEZADOS
  const headerRow1 = [
    'LISTADO COMPLETO DE ARTÍCULOS CONCILIADOS (COINCIDENCIA EXACTA)',
    '', '', '', '', '', '', '', ''
  ];
  ...
  const headerRow4 = [
    `Total Artículos Conciliados: ${articulosConciliados.length}`,
    `Total Unidades: ${this.calcularTotalUnidadesConciliadas()}`,
    `Valor Total: ${this.formatearMoneda(this.getValorTotalConciliados())}`,
    `Precisión: 100%`,
    '', '', '', '', ''
  ];
  
  const headerRow5 = [
    `Porcentaje del Inventario: ${this.getPorcentajeConciliados().toFixed(1)}%`,
    '', '', '', '', '', '', '', ''
  ];
  ...
  const columnHeaders = [
    '#', 'Código', 'Artículo', 'Stock Sistema', 'Conteo Físico',
    'Diferencia', 'Costo Unitario', 'Valor Total', 'Estado'
  ];

  data.push(headerRow1, headerRow2, headerRow3, headerRow4, headerRow5, headerRow6, columnHeaders);

  // DATOS
  articulosConciliados.forEach((item, index) => {
    const valorTotal = item.stock_disponible * item.costo_promedio;
  ...
  // FILA DE TOTALES
  const totalUnidades = this.calcularTotalUnidadesConciliadas();
  const totalValor = this.getValorTotalConciliados();
  
  data.push([
    '', '', 'TOTAL CONCILIADOS:', totalUnidades, totalUnidades, '0', '', totalValor, '100%'
  ]);
  ...
  data.push(['', '', '', '', '', '', '', '', '']);
  data.push([
    'ANÁLISIS DE ARTÍCULOS CONCILIADOS',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '✓ Coincidencia Perfecta: Conteo físico = Stock sistema',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '✓ Control Efectivo: Procesos de entrada/salida funcionando correctamente',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '✓ Sin Ajuste Requerido: No requieren corrección contable',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    `✓ Representa el ${this.getPorcentajeConciliados().toFixed(1)}% del inventario total`,
    '', '', '', '', '', '', '', ''
  ]);
  ...
  data.push([
    '1. Utilizar como benchmark para identificar procesos exitosos',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '2. Analizar características comunes para replicar buenas prácticas',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '3. Reconocer a responsables del manejo de estos artículos',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '4. Documentar procedimientos que generaron estos resultados',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    '5. Aplicar las mismas prácticas a artículos con diferencias',
    '', '', '', '', '', '', '', ''
  ]);
  ...
  data.push(['', '', '', '', '', '', '', '', '']);
  data.push([
    'DISTRIBUCIÓN POR GRUPO DE PRODUCTO',
    '', '', '', '', '', '', '', ''
  ]);
  data.push([
    'Grupo', 'Artículos', 'Unidades', 'Valor USD', '% Total', '', '', '', ''
  ]);

  const gruposConciliados = this.getAnalisisConciliadosPorGrupo();
  gruposConciliados.forEach(grupo => {
    const porcentaje = (grupo.valorConciliado / totalValor * 100).toFixed(1);
    data.push([
      grupo.grupo,
      grupo.articulosConciliados,
      grupo.unidadesConciliadas,
      grupo.valorConciliado,
      `${porcentaje}%`,
      '', '', '', ''
    ]);
  });
  ...
  const merges: Range[] = [
    ...
    { s: { r: 1, c: 0 }, e: { r: 1, c: 8 } }, // Bodega
    { s: { r: 2, c: 0 }, e: { r: 2, c: 8 } }, // Fecha
    { s: { r: 3, c: 0 }, e: { r: 3, c: 3 } }, // Totales header
    { s: { r: 4, c: 0 }, e: { r: 4, c: 8 } }  // Porcentaje
  ];
  
  // Combinar celdas de análisis
  const startRowAnalisis = articulosConciliados.length + 9;
  for (let i = 0; i < 5; i++) {
    merges.push({ s: { r: startRowAnalisis + i, c: 0 }, e: { r: startRowAnalisis + i, c: 8 } });
  }
  
  // Combinar celdas de recomendaciones
  const startRowRecom = startRowAnalisis + 7;
  for (let i = 0; i < 6; i++) {
    merges.push({ s: { r: startRowRecom + i, c: 0 }, e: { r: startRowRecom + i, c: 8 } });
  }
  
  // Combinar título de distribución
  const startRowDist = startRowRecom + 9;
  merges.push({ s: { r: startRowDist, c: 0 }, e: { r: startRowDist, c: 8 } });
  ...
  // CREAR WORKBOOK Y GUARDAR
  const workbook = { 
    Sheets: { 'Conciliados': worksheet }, 
    SheetNames: ['Conciliados'] 
  };
  ...
  const fecha = new Date().toLocaleDateString('es-EC').replace(/\//g, '-');
  const nombreArchivo = `Conciliados-${this.getNombreBodega()}-${fecha}`;
  
  this.guardarExcel(excelBuffer, nombreArchivo);
  this.mostrarMensajeExito('Excel de Artículos Conciliados generado exitosamente');
}
...
exportarAnalisisVarianzas(): void {
    ...
    const headerRow2 = [
      'CÓDIGO', 'ARTÍCULO', 'STOCK DISP.', 'COSTO PROM.', 'USUARIO CONTADOR', 'UBICACIÓN',
      ...this.gruposAnalisis.flatMap(() => [
        'Dif. vs Stock', 'Faltante x C.', 'Sobrante x C.', 'Total Contado'
      ]),
      'Conteo Final Aceptado', 'Dif. Final', 'Cantidad Faltante Total', 'Costo Total Faltante', 'Cantidad Sobrante Total', 'Costo Total Sobrante', 'ESTADO'
    ...
    // Combinar celdas del encabezado de DATOS DEL ARTÍCULO (ahora incluye 6 columnas)
    ...
    this.analisisVarianzas.forEach(articulo => {
      // Obtener usuario y ubicación del artículo
      const usuarioContador = this.getUsuarioContador(articulo.codigo_articulo);
      const ubicacion = this.getUbicacionArticulo(articulo.codigo_articulo);
  ...
  /**
   * Obtener el usuario que realizó el conteo de un artículo
   * Prioridad: Conteo días 6-9 > Reconteo día 10 > Reconteo día 11
   */
  getUsuarioContador(codigoArticulo: string): string {
    const articuloDiario = this.articulosDiarios.find(a => a.codigo_articulo === codigoArticulo);
    
    if (!articuloDiario) {
      return 'Sin Asignar';
    }

    // Buscar en días 6-9 primero (conteo principal)
    const dias6_9 = ['6', '7', '8', '9'];
    for (const fecha of this.fechasUnicas) {
      const dia = parseInt(fecha.split('-')[2], 10).toString();
      if (dias6_9.includes(dia)) {
        const conteos = articuloDiario.conteosPorDia[fecha];
        if (conteos && conteos.length > 0) {
          const usuario = conteos[0].usuario_contador;
          // Aplicar lógica de "sin asignar" -> "carlosm"
          if (!usuario || usuario.trim() === '' || 
              usuario.toLowerCase().includes('sin asignar') ||
              usuario.toLowerCase() === 'sinasignar' ||
              usuario.toLowerCase() === 'sin_asignar') {
            return 'carlosm';
          }
    ...
    // Si no hay conteo 6-9, buscar reconteo día 10
    const fecha10 = this.fechasUnicas.find(f => parseInt(f.split('-')[2], 10) === 10);
    if (fecha10) {
      const conteos = articuloDiario.conteosPorDia[fecha10];
      if (conteos && conteos.length > 0) {
        const usuario = conteos[0].usuario_contador;
        if (!usuario || usuario.trim() === '' || 
            usuario.toLowerCase().includes('sin asignar')) {
    ...
    // Si no hay día 10, buscar reconteo día 11
    const fecha11 = this.fechasUnicas.find(f => parseInt(f.split('-')[2], 10) === 11);
    if (fecha11) {
      const conteos = articuloDiario.conteosPorDia[fecha11];
      if (conteos && conteos.length > 0) {
        const usuario = conteos[0].usuario_contador;
        if (!usuario || usuario.trim() === '' || 
            usuario.toLowerCase().includes('sin asignar')) {
    ...
    // Si no hay ningún conteo, retornar carlosm por defecto
    return 'carlosm';
  }

  /**
   * Obtener la ubicación donde se encontró el artículo
   * Prioridad: Conteo días 6-9 > Reconteo día 10 > Reconteo día 11
   */
  getUbicacionArticulo(codigoArticulo: string): string {
    ...
    // Primero intentar obtener ubicación del campo 'ubicacion' del artículo
    if (articuloDiario.ubicacion && articuloDiario.ubicacion.trim() !== '') {
      return articuloDiario.ubicacion;
    }

    // Si no, buscar en los conteos (días 6-9 primero)
    const dias6_9 = ['6', '7', '8', '9'];
    for (const fecha of this.fechasUnicas) {
      const dia = parseInt(fecha.split('-')[2], 10).toString();
      if (dias6_9.includes(dia)) {
        const conteos = articuloDiario.conteosPorDia[fecha];
        if (conteos && conteos.length > 0) {
          const ubicacion = conteos[0].ubicacionconteo;
