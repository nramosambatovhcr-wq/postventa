...
@Component({
  selector: 'app-dashboardinvoice',
  templateUrl: './dashboardinvoice.component.html',
  styleUrls: ['./dashboardinvoice.component.css']
})
export class DashboardinvoiceComponent implements OnInit, OnDestroy  {
  stats = {
...
closeImageModal(): void {
  ...
  loadimportaciones() {
    this.loading = true;
    this.tutorialService.getInvoiceDetail(this.codigoCot).subscribe({
      next: (data: any) => {
...
rechazada(){
  ...
  downloadExcel(): void {
    ...
    // Save the file
    saveAs(blob, `Pedidos_Invoice_${new Date().toISOString().split('T')[0]}.xlsx`);
  }
