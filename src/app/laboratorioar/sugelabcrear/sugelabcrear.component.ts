...
@Component({
  selector: 'app-sugelabcrear',
  templateUrl: './sugelabcrear.component.html',
  styleUrls: ['./sugelabcrear.component.css']
})
export class SugelabcrearComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.pedidoService.createLabSugerido(pedidoData).subscribe({
      next: (response: any) => {
  ...
  uploadFiles(pedidoId: number): void {
    ...
    this.pedidoService.uploadLabSugeImages(pedidoId, formData).subscribe({
      next: () => {
        this.handleSuccess();
      },
      error: (error) => {
