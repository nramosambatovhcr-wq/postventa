...
@Component({
  selector: 'app-observacionesfilter',
  templateUrl: './observacionesfilter.component.html',
  styleUrls: ['./observacionesfilter.component.css']
})
export class ObservacionesfilterComponent implements OnInit, OnChanges, AfterViewChecked {
  ...
  private baseImageUrl = 'https://bodega.vehicentro.com:1830/api/api/Pedidos/ObservacionesFilter/imagen/';
  ...
  cargarObservaciones(): void {
    ...
    this.pedidoService.getObservacionesFilterPedido(this.pedido.idPedido).subscribe({
      next: (data: any) => {
  ...
  canAddObservation(): boolean {
    const permittedRoles = ['admin', 'repuestos', 'repuestoslv', 'repuestoslk', 'repuestosc','bodega','bodegaped','laboratorio1','laboratorio2','bodegaimpor'];
  ...
  enviarObservacion(): void {
    ...
    this.pedidoService.addObservacionFilter(formData).subscribe({
      next: (response) => {
