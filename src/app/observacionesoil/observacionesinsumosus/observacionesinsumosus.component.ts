...
@Component({
  selector: 'app-observacionesinsumosus',
  templateUrl: './observacionesinsumosus.component.html',
  styleUrls: ['./observacionesinsumosus.component.css']
})
export class ObservacionesinsumosusComponent implements OnInit, OnChanges, AfterViewChecked {
  ...
  private baseImageUrl = 'https://bodega.vehicentro.com:1830/api/api/Pedidos/ObservacionesInsumos/imagen/';
  ...
  cargarObservaciones(): void {
    ...
    this.pedidoService.getObservacionesInsumosPedido(this.pedido.idPedido).subscribe({
      next: (data: any) => {
  ...
  enviarObservacion(): void {
    ...
    this.pedidoService.addObservacionInsumos(formData).subscribe({
      next: (response) => {
