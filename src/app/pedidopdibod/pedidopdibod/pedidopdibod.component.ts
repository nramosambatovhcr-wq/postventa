...
export interface PedidoPdiDisplay {
  id_pedido?: number;
  codigo: string;
  descripcion: string;
  ...
  nombre?: string; // From join
  apellido?: string; // From join
  ultimo_mensaje?: string; // From LATERAL JOIN in backend
  fecha_ultimo_mensaje?: string; // From LATERAL JOIN in backend
  usuario_ultimo_mensaje?: string; // From LATERAL JOIN in backend
  observacionesCount?: number; // For tracking the number of observations
}

@Component({
  selector: 'app-pedidopdibod', // New selector for PDI Bodega
  templateUrl: './pedidopdibod.component.html',
  styleUrls: ['./pedidopdibod.component.css']
})
export class PedidopdibodComponent implements OnInit, OnDestroy {
  ...
  selectedPedidoId: number | null = null; // For the second observations modal if needed
  ...
  loadPedidosPdi() {
    this.loading = true;
    this.pedidosPdiService.getAllPedidosPdi().subscribe({ // Use the PDI service
      next: (data: PedidoPdiDisplay[]) => {
        this.allData = data;
        // Apply role-based filtering if necessary for PDI orders
  ...
  loadPedidosPdiPro() {
    ...
    this.loading = true;
    this.pedidosPdiService.getAllPedidosPdiPro().subscribe({ // Use the PDI service
      next: (data: PedidoPdiDisplay[]) => {
        this.allData = data;
        // Apply role-based filtering if necessary for PDI orders
        if (this.usrol === 'repuestoslv') {
  ...
  loadPedidosPdiAsig() {
    this.loading = true;
    this.pedidosPdiService.getAllPedidosPdiAsig().subscribe({ // Use the PDI service
      next: (data: PedidoPdiDisplay[]) => {
        this.allData = data;
        // Apply role-based filtering if necessary for PDI orders
        if (this.usrol === 'repuestoslv') {
  ...
  loadPedidosPdiRev() {
    this.loading = true;
    this.pedidosPdiService.getAllPedidosRev().subscribe({ // Use the PDI service
      next: (data: PedidoPdiDisplay[]) => {
        this.allData = data;
        // Apply role-based filtering if necessary for PDI orders
        if (this.usrol === 'repuestoslv') {
  ...
  // New function to open the second observations modal (if needed)
  verObservaciones(idPedido: any): void {
    this.selectedPedidoId = idPedido;
    // Assuming you have a separate component for detailed observations like app-observacionesped
    // this.showObservacionesModal1 = true; // Uncomment if you have this property and component
    console.log('Abriendo observaciones detalladas para pedido PDI:', idPedido);
  }

  // New function to close the second observations modal (if needed)
  closeObservacionesModal1(): void {
    // this.showObservacionesModal1 = false; // Uncomment if you have this property
  ...
  actualizarEstado(item: PedidoPdiDisplay): void {
    ...
    const pedidoActualizado: PedidoPdiData = {
      ...
      Cantidad: Number(item.cantidad) || 0, // Ensure number type
      Modelo: String(item.modelo || ""),
      Cliente: String(item.cliente || ""),
      Ot: String(item.ot || ""),
      Estado: String(item.estado || ""),
      IdUsuarioCreacion: Number(item.idUsuarioCreacion) || 0, // Ensure number type
      IdUsuarioModificacion: Number(this.id) || 0 // Use current user ID for modification
    };
  ...
  applyDateFilter() {
    if (!this.startDate || !this.endDate) {
      this.filteredData = this.allData; // Revert to all data if dates are cleared
    } else {
  ...
  pendiente() {
    ...
    this.router.navigate(['/pedidopdibod']); // Assuming this is the route for pending PDI
  ...
  revisado() {
    ...
    // And filter this.allData based on 'REVISADO' state
  }
  ...
  proceso() {
    ...
    // And filter this.allData based on 'PROCESO' state
  }
  ...
  asignado() {
    ...
    // And filter this.allData based on 'ASIGNADO' state
  }
  ...
  error() {
    ...
    // And filter this.allData based on 'ERRONEO' state
  }

  individual() {
    // Navigate to PDI individual order creation
    this.router.navigate(['/pedidopdicreate']);
    this.paginaActual = 'individual';
  }

  excel() {
    // Navigate to PDI Excel upload
    this.router.navigate(['/pedidopdiexcel']); // Assuming you'll create this component
   ...
   */
  downloadCSV(): void {
    let csvContent = 'Codigo,Descripcion,Cantidad,Observacion,Cliente,OT,Usuario,Fecha,Estado\n';
    this.filteredData.forEach(item => {
      const row = [
        ...
        item.estado || '-'
      ].map(field => `"${String(field).replace(/"/g, '""')}"`).join(','); // Handle commas and quotes in fields
      csvContent += row + '\n';
    });
