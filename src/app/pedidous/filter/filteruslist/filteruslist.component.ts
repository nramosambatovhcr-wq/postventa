...
export interface GroupedPedido {
  ...
  items: PedidoFilterDetail[]; // Array de los ítems individuales que componen este pedido agrupado
  idPedido?: number; // Referencia a un ID de pedido original para el título del modal, si se desea
  tiposIncluidos?: string[]; // Added for consistency, though not used in filter list logic currently
}

@Component({
  selector: 'app-filteruslist',
  templateUrl: './filteruslist.component.html',
  styleUrls: ['./filteruslist.component.css']
})
export class FilteruslistComponent implements OnInit, OnDestroy {
  searchTerm: string = '';
  currentPage: number = 1;
  itemsPerPage: number = 10;
  totalPages: number = 0;
  lista: GroupedPedido[] = []; // Ahora la lista es de pedidos agrupados
  allData: PedidoFilterDetail[] = []; // Todos los datos individuales de la API
  ...
  selectedPedido: PedidoOilDetail | null = null; // Changed to PedidoOilDetail, assuming a common interface or need for it here. Consider if this should be PedidoFilterDetail or a more generic type.
  id = 0;
    ...
    this.subscription.add(
      this.authService.usuarioActual$.subscribe(usuario => {
        ...
        if (this.usuario && this.usuario.id) {
          ...
          this.loadPedidosFilter(this.usuario.id);
        } else {
  ...
  /**
   * Loads filter orders for the current user from the API.
   * @param userId The ID of the user to fetch orders for.
   */
  loadPedidosFilter(userId: number): void {
    this.loading = true;
    this.errorMessage = null;

    this.pedidosOilService.getFilterPedidosByUserId(userId).subscribe({
      ...
      error: (e) => {
        console.error('Error al cargar pedidos de filtro:', e);
        this.errorMessage = 'Error al cargar la lista de pedidos de filtro.';
        this.allData = [];
        this.groupedData = [];
        this.filteredData = [];
        this.lista = [];
  ...
  /**
   * Groups individual PedidoFilterDetail items by Fecha Pedido and Nombre Proveedor.
   * @param data The array of individual PedidoFilterDetail items.
   * @returns An array of GroupedPedido objects.
   */
  private groupPedidosByDateAndSupplier(data: PedidoFilterDetail[]): GroupedPedido[] {
    ...
    data.forEach(item => {
      ...
      if (currentGroup) {
        ...
        // Add logic to determine included types if needed for Filter components later
        // Example: if (item.someProperty.includes('filterType')) { currentGroup.tiposIncluidos?.push('filterType'); }
      }
    });
    ...
    if (this.searchTerm && this.searchTerm.trim() !== '') {
      ...
      tempFilteredData = tempFilteredData.filter(group =>
        group.items.some(item => // Verifica si ALGÚN ítem dentro del grupo coincide
          (item.codigoFilter && item.codigoFilter.toLowerCase().includes(searchTermLower)) ||
          (item.descripcionFilter && item.descripcionFilter.toLowerCase().includes(searchTermLower)) ||
          (item.presentacionFilter && item.presentacionFilter.toLowerCase().includes(searchTermLower)) ||
  ...
  downloadExcel(): void {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'PedidosFiltros');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(blob, `PedidosFiltros_${new Date().toISOString().split('T')[0]}.xlsx`);
  }
  ...
  downloadCSV(): void {
    ...
    let csvContent = 'ID Pedido,Código Filtro,Descripción Filtro,Presentación,Cantidad Solicitada,Proveedor,Estado,Fecha Pedido,Fecha Modificación\n';
    ...
    saveAs(blob, `PedidosFiltros_${new Date().toISOString().split('T')[0]}.csv`);
  }

  individual(): void {
    this.router.navigate(['/filteruscreate']);
  }
  ...
  proceso(): void {
    this.filterByStatus('EN PROCESO');
  }
  enviado(): void {
  ...
  downloadModalExcel(): void {
    ...
    const workbook: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'DetallesPedidoFiltros');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    ...
    const filename = `DetallesPedidoFiltros_${fechaPedidoFormatted}_${this.selectedGroupedPedidoForDetails.nombreProveedor || 'sin_proveedor'}.xlsx`;
    const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  ...
  downloadModalCSV(): void {
    ...
    const filename = `DetallesPedidoFiltros_${fechaPedidoFormatted}_${this.selectedGroupedPedidoForDetails.nombreProveedor || 'sin_proveedor'}.csv`;
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8' });
    saveAs(blob, filename);
  }
}