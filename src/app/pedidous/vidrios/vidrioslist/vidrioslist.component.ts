...
import { OilService, PedidoFilterDetail, PedidoOilDetail, PedidoVidrioDetail } from 'src/app/services/oil.service';
...
export interface GroupedPedido {
  ...
  items: PedidoVidrioDetail[]; // Array de los ítems individuales que componen este pedido agrupado
...
@Component({
  selector: 'app-vidrioslist',
  templateUrl: './vidrioslist.component.html',
  styleUrls: ['./vidrioslist.component.css']
})
export class VidrioslistComponent implements OnInit, OnDestroy {
  ...
  allData: PedidoVidrioDetail[] = []; // Todos los datos individuales de la API
    ...
    this.subscription.add(
      this.authService.usuarioActual$.subscribe(usuario => {
        ...
        if (this.usuario && this.usuario.id) {
          ...
          this.loadPedidosVidrios(this.usuario.id);
        } else {
   ...
   */
  loadPedidosVidrios(userId: number): void {
    this.loading = true;
    this.errorMessage = null;

    this.pedidosOilService.getVidriosPedidosByUserId(userId).subscribe({
      ...
      error: (e) => {
        console.error('Error al cargar pedidos de vidrios:', e);
        this.errorMessage = 'Error al cargar la lista de pedidos de vidrios.';
        this.allData = [];
        this.groupedData = [];
        this.filteredData = [];
        this.lista = [];
   ...
   */
  private groupPedidosByDateAndSupplier(data: PedidoVidrioDetail[]): GroupedPedido[] {
    ...
    if (this.searchTerm && this.searchTerm.trim() !== '') {
      ...
      tempFilteredData = tempFilteredData.filter(group =>
        group.items.some(item => // Verifica si ALGÚN ítem dentro del grupo coincide
          (item.codigoVidrio && item.codigoVidrio.toLowerCase().includes(searchTermLower)) ||
          (item.descripcionVidrio && item.descripcionVidrio.toLowerCase().includes(searchTermLower)) ||
          (item.presentacionVidrio && item.presentacionVidrio.toLowerCase().includes(searchTermLower)) ||
  ...
  downloadCSV(): void {
    ...
    flatDataForExport.forEach(item => {
      const row = [
        item.idPedido || '',
        item.codigoVidrio || '',
        item.descripcionVidrio || '',
        item.presentacionVidrio || '-',
  ...
  individual(): void {
    this.router.navigate(['/vidriouscreate']);
  }
  ...
  downloadModalExcel(): void {
    ...
    const itemsForExport = this.selectedGroupedPedidoForDetails.items.map(item => ({
      ...
      'Código': item.codigoVidrio,
      'Descripción': item.descripcionVidrio,
      'Presentación': item.presentacionVidrio || '-',
