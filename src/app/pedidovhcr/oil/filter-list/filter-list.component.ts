...
// Ensure PedidoFilterDetail has 'agencia' if you want to group by it.
// Assuming PedidoFilterDetail has 'agencia' like PedidoOilDetail.
import { OilService, PedidoFilterDetail, PedidoOilDetail } from 'src/app/services/oil.service'; 
...
export interface GroupedPedido {
  ...
  items: PedidoFilterDetail[];
  idPedido?: number; // This might be the first item's ID or a conceptual group ID
  totalItems?: number; // To show the total number of items in the group
}

// Keep Filter1 interface for compatibility if needed elsewhere
...
export class FilterListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private oilService: OilService, // Renamed from filterService if it was different
  ...
  actualizarEstadoPedido(group: GroupedPedido): void {
    ...
    // It seems 'idPedido' is a common field for all items in a group, or it refers to the first item's ID.
    // If you intend to update each item's status individually, iterate through group.items.
    // Assuming updateFilterRequestStatus takes an item ID and new status:
    group.items.forEach((item: PedidoFilterDetail) => {
      if (item.idPedido) { // Ensure item.idPedido exists
        console.log('Actualizando estado para item de filtro:', item.idPedido, 'Estado:', group.estadoPedido);
        this.oilService.updateFilterRequestStatus(item.idPedido, group.estadoPedido || 'PENDIENTE') // Provide a default status if group.estadoPedido is undefined
          .subscribe({
            next: (response) => {
              console.log(`Estado actualizado para item de filtro ${item.idPedido}:`, response);
              // You might want to update the specific item's status in the local data here
            },
            error: (error) => {
              console.error(`Error al actualizar item de filtro ${item.idPedido}:`, error);
              this.showToast(`Error al actualizar item ${item.idPedido}: ${error.error || error.message}`, 'error');
            }
          });
      } else {
        console.warn('Item de filtro sin idPedido, no se puede actualizar:', item);
      }
    });
  ...
  /**
   * Groups individual PedidoFilterDetail items by Date, Supplier, User, and Agency
   */
  private groupPedidosByDateSupplierUserAgency(data: PedidoFilterDetail[]): GroupedPedido[] {
    ...
    data.forEach(item => {
      ...
      const agencyKey = (item as any).agencia?.trim() || 'sin-agencia'; // Cast to any if 'agencia' is not in PedidoFilterDetail yet
   ...
   */
  private determineGroupStatus(items: PedidoFilterDetail[]): string {
    ...
    if (this.searchTerm.trim() !== '') {
      ...
      tempFilteredData = tempFilteredData.filter(group =>
        ...
        group.items.some(item =>
          (item.codigoFilter?.toLowerCase().includes(searchTermLower)) ||
          (item.descripcionFilter?.toLowerCase().includes(searchTermLower)) ||
          (item.presentacionFilter?.toLowerCase().includes(searchTermLower)) ||
  ...
  downloadExcel(): void {
    ...
    const flatDataForExport = this.filteredData.flatMap(group =>
      group.items.map(item => ({
        ...
        'Código Filtro': item.codigoFilter || '',
        'Descripción': item.descripcionFilter || '',
        'Presentación': item.presentacionFilter || '-',
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'PedidosFiltros'); // Renamed sheet
    ...
    const filename = `PedidosFiltros_${new Date().toISOString().split('T')[0]}.xlsx`; // Renamed filename
  ...
  downloadCSV(): void {
    ...
    let csvContent = 'ID Pedido,Código Filtro,Descripción,Presentación,Cantidad Solicitada,Proveedor,Usuario Solicitante,Agencia,Estado,Fecha Pedido,Fecha Modificación\n'; // Updated header
    
    flatDataForExport.forEach(item => {
      const row = [
        item.idPedido || '',
        item.codigoFilter || '',
        item.descripcionFilter || '',
        item.presentacionFilter || '-',
    ...
    const filename = `PedidosFiltros_${new Date().toISOString().split('T')[0]}.csv`; // Renamed filename
    saveAs(blob, filename);
  ...
  downloadModalExcel(): void {
    ...
    const workbook: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'DetallesPedidoFiltro'); // Renamed sheet
    ...
    const filename = `DetallesPedidoFiltro_${fechaFormatted}_${this.selectedGroupedPedidoForDetails.nombreProveedor || 'sin_proveedor'}.xlsx`; // Renamed filename
    const blob = new Blob([excelBuffer], {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    });
  ...
  downloadModalCSV(): void {
    ...
    const filename = `DetallesPedidoFiltro_${fechaFormatted}_${this.selectedGroupedPedidoForDetails.nombreProveedor || 'sin_proveedor'}.csv`; // Renamed filename
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8' });
  ...
  editOil(filter: Filter1): void { // Renamed parameter to filter
    console.log('Editando filtro:', filter);
    this.router.navigate(['/filter-edit', filter.idFilter]); // Changed route and ID
  }

  deleteOil(idFilter: number | undefined): void { // Renamed parameter to idFilter
    if (idFilter === undefined) {
      console.error('Cannot delete filter: ID is undefined.');
      return;
    }
