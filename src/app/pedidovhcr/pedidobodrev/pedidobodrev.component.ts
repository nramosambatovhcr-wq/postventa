...
import { Pedido } from '../pedidobod/pedidobod.component';


@Component({
  selector: 'app-pedidobodrev',
  templateUrl: './pedidobodrev.component.html',
  styleUrls: ['./pedidobodrev.component.css']
})
export class PedidobodrevComponent implements OnInit, OnDestroy  {
  stats = {
...
getEstadoDisplay(estado: string): string {
  ...
  loadimportaciones() {
    this.loading = true;
    this.tutorialService.revisados().subscribe({
      next: (data: any) => {
  ...
  applySearchFilter(resetData: boolean = true) {
    ...
    if (searchTermLower === '') {
      ...
      const dataToFilter = resetData ? [...this.allData1] : this.filteredData;
