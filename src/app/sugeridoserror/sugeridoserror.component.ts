...
@Component({
  selector: 'app-sugeridoserror',
  templateUrl: './sugeridoserror.component.html',
  styleUrls: ['./sugeridoserror.component.css']
})
export class SugeridoserrorComponent implements OnInit, OnDestroy  {
  stats = {
...
closeImageModal(): void {
  ...
  loadimportaciones() {
    this.loading = true;
    this.tutorialService.errosuge2(this.id).subscribe({
      next: (data: any) => {
...
error(){
  this.router.navigate(['/sugeridosrev']);
}
