...
@Component({
  selector: 'app-sugeridosrev',
  templateUrl: './sugeridosrev.component.html',
  styleUrls: ['./sugeridosrev.component.css']
})
export class SugeridosrevComponent implements OnInit, OnDestroy  {
  stats = {
...
closeImageModal(): void {
  ...
  loadimportaciones() {
    this.loading = true;
    this.tutorialService.revsuge1(this.id).subscribe({
      next: (data: any) => {
...
error(){
  this.router.navigate(['/sugeridoserror']);
}

individual(){
  this.router.navigate(['/crearsugeridos']);
}
sugerido(){
  this.router.navigate(['/pedidosbodinser']);
}
excel(){
this.router.navigate(['/sugeridosexcel']);
}
